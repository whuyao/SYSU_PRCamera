...
static image_list_t *create_gabor(int nscales, const int *or1, int width, int height)
{
    int i, j, fn;
    ...
    int nfilters = 0;
    for(i=0;i<nscales;i++)  nfilters+=or1[i];
    ...
    for(i = 1; i <= nscales; i++)
    {
        for(j = 1; j <= or1[i-1]; j++)
        {
            ...
            param[l][2] = 16*(double)pow(double(or1[i-1]), 2)/(double)pow(32.0f, 2);
            param[l][3] = M_PI/(or1[i-1])*(j-1);
            l++;
        }
    }
